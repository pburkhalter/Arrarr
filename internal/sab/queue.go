...
func (s *Server) handleHistory(w http.ResponseWriter, r *http.Request) {
	...
	for _, j := range jobs {
		...
		case job.StateReady:
			...
			// The puller wrote files to local_path under DOWNLOAD_DIR; that's
			// the path Sonarr/Radarr should import from.
			if j.LocalPath.Valid && j.LocalPath.String != "" {
				slot.Storage = j.LocalPath.String
				slot.Path = j.LocalPath.String
			}
